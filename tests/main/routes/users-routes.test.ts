...
describe("Users Routes", () => {
  ...
  const makeToken = async () => {
    ...
    await postgresClient.query(
      ...
      ["123123123", params.email, params.name, params.password, date, date],
    );
  ...
  describe("POST /users", () => {
    ...
    it("Should return 500 if usecase throw some error", async () => {
      jest
        .spyOn(PostgresHelper.client.user, "create")
    ...
    it("Should return 500 if user already exists", async () => {
      ...
      await postgresClient.query(
        ...
        ["123123123", params.email, params.name, params.password, new Date()],
      );
      await request(app)
        .post("/api/users")
        .send(params)
  ...
  describe("GET /users", () => {
    ...
    it("Should return 403 if bad formate token is provided", async () => {
      await request(app)
        .get("/api/users?page=1&limit=10")
    ...
    it("Should return 500 if usecase throw some error", async () => {
      jest
        .spyOn(PostgresHelper.client.user, "findMany")
        .mockRejectedValueOnce(new Error());
      const { token: accessToken } = await makeToken();
