...
describe("Rooms Routes", () => {
  ...
  describe("GET /rooms", () => {
    ...
    it("Should return 200 with rooms", async () => {
      ...
      await request(app)
        ...
        .expect([
          {
            ...databaseRoom,
            createdAt: databaseRoom.createdAt.toISOString(),
            updatedAt: databaseRoom.updatedAt.toISOString(),
          },
        ]);
    });
    ...
    it("Should return 200 with rooms in the limit", async () => {
      const authorization = await makeToken();
      const mockRooms = [
        makeRoom({ ownerId: authorization.id }),
        makeRoom({ ownerId: authorization.id }),
        makeRoom({ ownerId: authorization.id }),
      ];
    ...
    it("Should return 200 with rooms in the limit and name filter", async () => {
      ...
      await request(app)
        .get(
          `/api/rooms?page=1&limit=2&name=${mockRooms[0].name.split(" ")[0]}`,
        )
        .set("x-access-token", authorization.token)
        .expect(200)
        .expect([
          {
            ...mockRooms[0],
            createdAt: mockRooms[0].createdAt.toISOString(),
            updatedAt: mockRooms[0].updatedAt.toISOString(),
          },
        ]);
    });
