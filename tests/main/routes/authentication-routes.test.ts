...
describe("Authentication Routes", () => {
  ...
  const makeUserPostgres = async (user: User) => {
    ...
    const userRow = await postgresClient.query(
      `INSERT INTO "User" (id, email, name, password, "createdAt", "updatedAt") VALUES ('${user.id}', '${user.email}', '${user.name}', '${hashPassword}', '${user.createdAt.toISOString()}', '${user.updatedAt.toISOString()}')`,
    );
    return userRow;
  };
  ...
  describe("/Auth route", () => {
    describe("POST /login", () => {
      ...
      it("Should return 500 if some error occurs", async () => {
        jest
          .spyOn(PostgresHelper.client.user, "findUnique")
    ...
    describe("GET /refresh", () => {
      ...
      it("Should return 500 if some error occurs", async () => {
        ...
        jest
          .spyOn(PostgresHelper.client.user, "findFirst")
          .mockImplementationOnce(() => {
